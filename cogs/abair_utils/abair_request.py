...
async def get_pronunciation_text(input_string, dialect):
    text_request_payload = {"dialect": dialect, "inputText": input_string, "synth-mode": "dnn", "speed": "1.0",
                            "pitch": "1.0", "speaker": "female"}
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post("https://abair.ie/action/synthesize", cookies=request_cookies,
...
async def get_pronunciation_voice(input_string, dialect):
...
async def get_pronunciation(input_string, dialect):
    pronounciation_voice_promise = get_pronunciation_voice(input_string, dialect)
    pronounciation_text_promise = get_pronunciation_text(input_string, dialect)
